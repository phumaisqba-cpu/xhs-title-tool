...
class LLMClient:
    ...
    def __init__(self, provider: str = "glm-4-flash"):
        ...
        # 优先读 Streamlit Secrets（云端部署），fallback 到 .env（本地开发）
        self.api_key = self._resolve_api_key(config["header_key"])
        
        if not self.api_key:
            raise ValueError(
                f"❌ {config['header_key']} 未配置！\n"
                f"  • 云端部署：到 Streamlit Cloud → Settings → Secrets 填入\n"
                f"  • 本地运行：复制 .env.example → .env 并填入你的 API_KEY\n"
    ...
    def _resolve_api_key(self, key_name: str) -> str:
        """
        解析 API key 优先级：
          1. Streamlit Secrets（部署在 share.streamlit.io）
          2. 环境变量 / .env（本地开发）
        """
        # 1. 试 Streamlit secrets
        try:
